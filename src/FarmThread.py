    ...
    def run(self):
        ...
        try:
            ...
            if self.browser.login(
                self.config.getAccount(self.account)["username"],
                ...
                self.config.getAccount(self.account)["imapServer"],
...
def getLeagues():
    ...
    res = requests.get(
        "https://esports-api.lolesports.com/persisted/gw/getLeagues?hl=en-GB",
        headers=headers
    )
