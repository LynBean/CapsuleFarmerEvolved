...
from .Exceptions import Fail2FAException
from .Exceptions import FailFind2FAException
from .Exceptions import InvalidIMAPCredentialsException
from .Exceptions import NoAccessTokenException
...
class Browser:
    ...
    def login(
        ...
        try:
            ...
            if "multifactor" in resJson.get("type", ""):
                if (imapserver != ""):
                    ...
                    data = {
                        "type": "multifactor", "code": req.code, "rememberDevice": True
                    }
                    res = self.client.put(
                        "https://auth.riotgames.com/api/v1/authorization",
                        json=data
                    ...
                    twoFactorCode = self.get2FACode()
                    self.stats.updateStatus(self.account, f"[green]CODE SENT")
                    data = {
                        "type": "multifactor", "code": twoFactorCode, "rememberDevice": True
                    }
                    res = self.client.put(
                        "https://auth.riotgames.com/api/v1/authorization",
                        json=data
        ...
        if token and state:
            ...
            self.client.post(
                "https://login.riotgames.com/sso/login",
                data=data
            ).close()
            self.client.post(
                "https://login.lolesports.com/sso/login",
                data=data
            ).close()
            self.client.post(
                "https://login.playvalorant.com/sso/login",
                data=data
            ).close()
            self.client.post(
                "https://login.leagueoflegends.com/sso/callback",
                data=data
            ).close()
            self.client.get(
    ...
    def get2FACode(self) -> str:
        try:
            return input(f"Enter 2FA code for {self.account}:\n")

        except EOFError:
            pass

        # Alternative method if the terminal doesn't accept input
        path = makePath("2fa.txt")

        with open(str(path), "w+") as f:
            pass

        print("A file named 2fa.txt has been created in the current directory. Please enter your 2FA code in this file and save it.")
    ...
    def __needSessionRefresh(self) -> bool:
        ...
        res = jwt.decode(
            self.client.cookies.get_dict()["access_token"],
            options={
    ...
    def __sendWatch(self, match: Match):
        ...
        res = self.client.post(
            "https://rex.rewards.lolesports.com/v1/events/watch",
            json=data,
            headers=headers
        )
    ...
    def __dumpCookies(self):
        with open(
            str(makePath(f'sessions/{self.account}.saved')),
            'wb'
        ) as f:
            pickle.dump(self.client.cookies, f)

    def __loadCookies(self):
        path = makePath(f'sessions/{self.account}.saved')
